...
class Request():
    ...
    def send_request(self, socket, dialog_box, general, header, request_id=""):
        ...
        file_to_run = request_dict["Run File"]
        if file_to_run.endswith("\n"):
            file_to_run = file_to_run[:-1]
        header += "::Request::"
        ...
        executors_amount = RequestFileCom.get_value("Executors Amount")
        if executors_amount.endswith("\n"):
            executors_amount = executors_amount[:-1]

        # Send the Commitments and Privileges values.
        socket.send(header + "Uploading::Executors Amount::" + executors_amount)
