...
class Request():
    ...
    def send_request(self, socket, dialog_box, general, header, request_id=""):
        ...
        request_dict = RequestFileCom.file_to_dict(path="Requests/" + "".join(self.file_to_run.split(".")[:-1]) +
                                                        "/current_request.txt")
        ...
        socket.send(header + "Uploading::Executors Amount::" + RequestFileCom.get_value(
            "Executors Amount", path="Requests/" + "".join(self.file_to_run.split(".")[:-1]) +
...
class ServerRequest(Request):
    ...
    # Static variable for the ids of the requests.
    next_id = 1
    id_lock = Lock()
    ...
    def __init__(self, client, file_to_run, dir_name):
        ...
        ServerRequest.id_lock.acquire()
        self.id = ServerRequest.next_id
        ServerRequest.next_id += 1
        ServerRequest.id_lock.release()
    ...
    def find_executors(self):
        ...
        if not self.request_died:
            ...
            if len(can_execute) >= self.executors_amount:
                ...
                executors_ids = ', '.join([str(executor.id) for executor in self.executors])

                self.client.client_communicator.send.messages_to_send.append("Request::Status::Enough available "
                                                                             "computers::" + executors_ids)
    ...
    def abort_request(self):
        """
            The function sends to all the executors that are responsible on the execution and to the client of the
            request that the request has finished.
        """

        # Send the message to the client.
        self.client.abort_request()

        # Send the message to the executors.
        for executor in self.executors:

            executor.abort_request(self)
...
class ExecutorRequest(Request):
    ...
    def __init__(self, file_to_run, dir_name, request_id=0):
        ...
        # The request of the request as it's represented in the system.
        self.id = request_id
